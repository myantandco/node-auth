...
module.exports = function({jwtSecret}) {
    ...
    return function (req, res, next) {
        ...
        if (splitHeaderArr.length !== 2) {
            ...
            return res.status(401).send(formatError("Invalid Authorization Header"));
        }
        ...
        if (authType !== 'Bearer') {
            ...
            return res.status(401).send(formatError('Authorization type must be Bearer'));
        }
        ...
        try {
            ...
            return res.status(401).send(formatError('Failed to authenticate. Invalid JWT.' + (clientMessage ? ` Reason: ${clientMessage}` : "")));
        }
    }
}
